...
local function rearrangeTable()
    for _, v in pairs(t) do
        ...
        if move then
            table.move(stacks[from], #stacks[from] - move+1, #stacks[from], #stacks[to]+1, stacks[to])
            for i = 1, move do  -- fix for: table.move does not remove values if moved within the same table (?)
